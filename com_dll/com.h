...
public:
	Com(){
		...
		InitializeCriticalSection(&this->lockx);
	...
	virtual ~Com(){
		...
		DeleteCriticalSection(&this->lockx);
	}
	...
	int read(unsigned char* buffer, int readl)
	{
		EnterCriticalSection(&this->cs);
		DWORD wCount = readl;
		BOOL bReadStat;

		bReadStat = ReadFile(hcom, buffer, wCount, &wCount, NULL);
		if (!bReadStat)
		{
	...
	bool write(const unsigned char* buffer, int len)
	{
		...
		if (!bWriteStat || dwBytesWrite != len){
			LeaveCriticalSection(&this->cs);
			return false;
		}
		PurgeComm(hcom, PURGE_TXABORT | PURGE_RXABORT | PURGE_TXCLEAR | PURGE_RXCLEAR);
